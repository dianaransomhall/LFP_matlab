...
if ~mac
    ...
    load('/Volumes/BACKUP/EXTRAP/matlab_Cina_Herr_Diana/finshed_data/04_12_2016_lindanelow.mat');
    load('/Volumes/BACKUP/EXTRAP/matlab_Cina_Herr_Diana/finshed_data/02_15_2016_conlin.mat');
end;

% make analysis data set
filepath.linlow=linlow.filepath;
filepath.conlin=con.filepath;
chem=linlow;  
cont=con; cont.file = con.file ;
nFiles=length(cont.file) ; 
...
LINLOWanova = make_t_anova_data(chem, cont, nFiles, want) ;
% run analysis
LINLOW = do_t_anova(LINLOWanova, want ) ; % LIN{1}
clear('linlow');clear('con');
...
if ~mac
 ...
 load('F:\EXTRAP\matlab_Cina_Herr_Diana\finshed_data\8_9_2014_contri.mat');
    ...
    load('/Volumes/BACKUP/EXTRAP/matlab_Cina_Herr_Diana/finshed_data/8_09_2016_tri65.mat');
    load('/Volumes/BACKUP/EXTRAP/matlab_Cina_Herr_Diana/finshed_data/7_21_2016_contri.mat');
end;

% make analysis data set
filepath.tri65=tri65.filepath;
cont={};
conFileStart=8;
for curF=conFileStart:(length(tri65.filepath)+conFileStart-1)
   filepath.conh20=con.filepath;
   ind=curF-conFileStart+1 ;
   cont.filepath{ ind }=con.filepath{ curF } ;
   cont.file( ind )=con.file( curF ) ;
end;
cont.fbands=con.fbands;

chem=tri65;  
nFiles=length(cont.file) ; 
want.sec=0; want.ch=0; want.well=0;want.diff=0;want.pchange=1;

TRI65anova = make_t_anova_data(chem, cont, nFiles, want) ;
% run analysis
TRI65 = do_t_anova(TRI65anova, want ) ;
clear('tri65');clear('con');
...
if ~mac
 ...
 load('F:\EXTRAP\matlab_Cina_Herr_Diana\finshed_data\8_9_2014_contri.mat');
    ...
    load('/Volumes/BACKUP/EXTRAP/matlab_Cina_Herr_Diana/finshed_data/7_21_2016_tri.mat');
    load('/Volumes/BACKUP/EXTRAP/matlab_Cina_Herr_Diana/finshed_data/7_21_2016_contri.mat');
end;

% make analysis data set
filepath.tri5=tri.filepath;
filepath.conh20=con.filepath;
chem=tri;  
cont=con; cont.file = con.file ;
nFiles=length(cont.file) ; 
...
TRI5anova = make_t_anova_data(chem, cont, nFiles, want) ;
% run analysis
TRI5 = do_t_anova(TRI5anova, want ) ;
clear('tri');clear('con');
...
%% Save results
 ...
 if ~mac
    filename='F:\EXTRAP\matlab_Cina_Herr_Diana\finshed_data\01_03_2015_pChangeanova.mat' ;
     ...
     % filename='/Volumes/BACKUP/EXTRAP/matlab_Cina_Herr_Diana/finshed_data/04_12_2016_pChangeanova.mat' ;
     filename='/Volumes/BACKUP/EXTRAP/matlab_Cina_Herr_Diana/finshed_data/09_01_2016_pChangeanova.mat' ;
 end;
 save(filename, 'BIC','BICanova','ACE', 'ACEanova', ...
     ...
     'LINLOW', 'LINLOWanova', 'GLY', 'GLYanova','H20','H20anova',...
     'PER25', 'PER25anova', 'PER50', 'PER50anova','TRI65','TRI65anova', ...
     'TRI5','TRI5anova','filepath') ;
...
wantData=1;
if wantData
    load('/Volumes/BACKUP/EXTRAP/matlab_Cina_Herr_Diana/finshed_data/09_01_2016_pChangeanova.mat');
end;



    xlsfilestr='/Volumes/BACKUP/EXTRAP/documents/Results/lfp_results_t-testVehicleCont_EqVariances_09_01_2016.csv';
    title1={'Results: % Baseline chem vs. % baseline vehicle control, mean all Electrodes, t-test: eq. var'};
    title=title1{1};
    %da,gly,ace,tri are H20 controls
    %bic, car, per30, per50 are dmso controls
    chemName= {'ace','bic', 'da','dmso','car5','car15', 'car30','gly', 'h20',...
        'lin', 'linlow', 'per25','per50','tri5', 'tri65' };
    ...
    % write files
        ...
        elseif strcmp( chemName{1,k},'tri5')
            anovaData = TRI5anova ; test = TRI5;
            for i=1:length(filepath.tri5)
                 if mac
                    temp=strsplit( filepath.tri5{i} , '\');
                    n1=temp{end};
                else
                  [p,n1,x] = fileparts( filepath.tri5{i} );
                end;
            ...
            [anova{curB}.t, anova{curB}.tp_value, ...
                anova{curB}.ci, anova{curB}.stats ]=...
                ttest2(TRI5anova{curB}.pchange, H20anova{curB}.pchange ) %significant
            end;
            nlines=length(filepath.tri5 );
            
        elseif strcmp( chemName{1,k},'tri65')
            anovaData = TRI65anova ; test = TRI65;
            for i=1:length(filepath.tri65)
                 if mac
                    temp=strsplit( filepath.tri65{i} , '\');
                    n1=temp{end};
                else
                  [p,n1,x] = fileparts( filepath.tri65{i} );
                end;
            ...
            [anova{curB}.t, anova{curB}.tp_value, ...
                anova{curB}.ci, anova{curB}.stats ]=...
                ttest2(TRI65anova{curB}.pchange, H20anova{curB}.pchange ) %significant
            end;
            nlines=length(filepath.tri65 );
        ...
        elseif strcmp( chemName{1,k},'lin')
         ...
         elseif strcmp( chemName{1,k},'linlow')
            anovaData = LINLOWanova ; test = LINLOW ;
            for i=1:length(filepath.linlow)
                [p,n1,x] = fileparts( filepath.linlow{i} );
                n{i}=n1 ;
            end;
            for curB=1:5
            [anova{curB}.t, anova{curB}.tp_value, ...
                anova{curB}.ci, anova{curB}.stats ]=...
                ttest2(LINLOWanova{curB}.pchange, DMSOanova{curB}.pchange ) %significant
            end;
            CONanova=DMSOanova;
            nlines=length(filepath.linlow );
...
if ~mac
    ...
    xlsfilestr='/Volumes/BACKUP/EXTRAP/documents/Results/lfp_results_t-testvVehicleControl_UnEqVar_09_01_2016.csv';
end;
...
% write files
    ...
    elseif strcmp( chemName{1,k},'tri5')
        ...
        [anova{curB}.t, anova{curB}.tp_value, ...
            anova{curB}.ci, anova{curB}.stats ]=...
            ttest2(TRI5anova{curB}.pchange, DMSOanova{curB}.pchange,'vartype','unequal' ) %significant
        end;
        CONanova=H20anova;
        nlines=length(filepath.tri5 );
    ...
    elseif strcmp( chemName{1,k},'tri65')
        ...
        [anova{curB}.t, anova{curB}.tp_value, ...
            anova{curB}.ci, anova{curB}.stats ]=...
            ttest2(TRI65anova{curB}.pchange, DMSOanova{curB}.pchange,'vartype','unequal' ) %significant
        end;
        CONanova=H20anova;
        nlines=length(filepath.tri65 );
   ...
   elseif strcmp( chemName{1,k},'lin')
    ...
    elseif strcmp( chemName{1,k},'linlow')
        ...
        [anova{curB}.t, anova{curB}.tp_value, ...
            anova{curB}.ci, anova{curB}.stats ]=...
            ttest2(LINLOWanova{curB}.pchange, DMSOanova{curB}.pchange,'vartype','unequal' ) %significant
        end;
        CONanova=DMSOanova;
        nlines=length(filepath.linlow );
