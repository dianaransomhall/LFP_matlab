...
%%%+++++++++++++CAR+++++++++++++++
% load data
load('F:\EXTRAP\matlab_Cina_Herr_Diana\finshed_data\10_30_2014_car30.mat');
load('F:\EXTRAP\matlab_Cina_Herr_Diana\finshed_data\11_05_2014_carcon.mat');
chem=car;   %which files in con are CAR baseline
cont=con; 
nFiles=length(cont.file) ; 
want.sec=0; want.ch=0; want.well=0; want.diff=0;want.pchange=1;
% get filepaths for writing
filepath.car=car.filepath;
filepath.concar=con.filepath;

CARanova = make_t_anova_data_AEOnly(chem, cont, nFiles, want) ;
% run analysis
...
load('F:\EXTRAP\matlab_Cina_Herr_Diana\finshed_data\10_30_2014_per30con.mat');
chem=per; 
cont=con; 
nFiles=length(cont.file) ; 
want.sec=0; want.ch=0; want.well=0; want.diff=0;want.pchange=1;
...
filepath.conper=con.filepath;

PERanova = make_t_anova_data_AEOnly(chem, cont, nFiles, want) ;
% run analysis
...
chem=da;   %which files in con are PER baseline
cont=con; 
nFiles=length(cont.file) ; 
want.sec=0; want.ch=0; want.well=0; want.diff=0;want.pchange=1;

DAanova = make_t_anova_data_AEOnly(chem, cont, nFiles, want) ;
% run analysis
...
%%%+++++++++++++PERM 50 uM+++++++++++++++
% load data
load('F:\EXTRAP\matlab_Cina_Herr_Diana\finshed_data\10_21_2014_per50.mat');
load('F:\EXTRAP\matlab_Cina_Herr_Diana\finshed_data\10_21_2014_conPER50.mat');
% make analysis data set
filepath.per50=per.filepath;
filepath.per50con=con.filepath;
chem=per;  conFIndex=[1:6]; %which files in con are PER baseline
cont=con; cont.file = con.file(conFIndex) ;
nFiles=length(cont.file) ; 
want.sec=0; want.ch=0; want.well=0; want.diff=0;want.pchange=1;

PER50anova = make_t_anova_data_AEOnly(chem, cont, nFiles, want) ;
% run analysis
...
%%%+++++++++++++Ace+++++++++++++++
% load data
load('F:\EXTRAP\matlab_Cina_Herr_Diana\finshed_data\10_30_2014_ace.mat');
load('F:\EXTRAP\matlab_Cina_Herr_Diana\finshed_data\11_04_2014_conace.mat');
% make analysis data set
...
ACEanova = make_t_anova_data_AEOnly(chem, cont, nFiles, want) ;
% run analysis
...
%%%+++++++++++++H20+++++++++++++++
% load data
load('F:\EXTRAP\matlab_Cina_Herr_Diana\finshed_data\10_30_2014_h20.mat');
load('F:\EXTRAP\matlab_Cina_Herr_Diana\finshed_data\10_30_2014_conh20.mat');

% make analysis data set
filepath.h20=h20.filepath;
filepath.conh20=con.filepath;
chem=h20;   %which files in con are PER baseline
cont=con; 
nFiles=length(cont.file) ; 
want.sec=0; want.ch=0; want.well=0; want.diff=0;want.pchange=1;

H20anova = make_t_anova_data_AEOnly(chem, cont, nFiles, want) ;
% run analysis
...
TRIanova = make_t_anova_data_AEOnly(chem, cont, nFiles, want) ;
% run analysis
...
% load data
 load('F:\EXTRAP\matlab_Cina_Herr_Diana\finshed_data\10_28_2014_gly.mat');
 load('F:\EXTRAP\matlab_Cina_Herr_Diana\finshed_data\10_28_2014_congly.mat');

%+++++++++ t-test or anova
%+++++++  BIC +++++++++++++++++++++++
chem=gly; conFIndex=[1:8]; %index of con files
cont=con; cont.file = con.file(conFIndex) ;
nFiles=length(cont.file) ; 
want.sec=0; want.ch=0; want.well=0; want.diff=0;want.pchange=1;

filepath.gly=gly.filepath;
filepath.congly=con.filepath(conFIndex) ;

GLYanova = make_t_anova_data_AEOnly(chem, cont, nFiles, want) ;
...
% filename='F:\EXTRAP\matlab_Cina_Herr_Diana\finshed_data\11_05_2014_pChangeTtest.mat';
 save(filename, 'PER', 'PERanova', 'DA','DAanova',...
    'TRI', 'TRIanova', 'BIC','BICanova', 'CAR','CARanova',...
    'PER50','PER50anova', 'H20','H20anova', 'ACE','ACEanova', ...
    'GLY', 'GLYanova', 'filepath') ;
...
% ++++++++++++++++++++++++++++++++++++++
% write results
xlsfilestr='F:\EXTRAP\documents\Results\lfp_AEOnlyResults_11-06-2014.xls';
title='Results of Mean % Control in Power averaged across All Electrodes';
xlswrite(xlsfilestr,title, 1,'A1' ) ;
% 
chemName= {'bicuculline','permethrin25','carbaryl30','permethrin50',...
    'DomoicAcid','Acetaminophen','H20', 'TRI', 'GLY' }
for k=1:size(chemName,2)
% write files
    if strcmp( chemName{1,k},'permethrin25' )
        startCell=3;
    ...
    elseif strcmp( chemName{1,k},'carbaryl30')
        startCell=11;
        ...
        [p,n6,x] = fileparts( filepath.car{6} );
        [p,n7,x] = fileparts( filepath.car{7} );
        [p,n8,x] = fileparts( filepath.car{8} );
    ...
    elseif strcmp( chemName{1,k},'DomoicAcid')
        startCell=29;
        ...
        [p,n6,x] = fileparts( filepath.da{6} );
    ...
    elseif strcmp( chemName{1,k},'permethrin50')
        ...
        [p,n4,x] = fileparts( filepath.per50{4} );
        [p,n5,x] = fileparts( filepath.per50{5} );
        [p,n6,x] = fileparts( filepath.per50{6} );
    ...
    elseif strcmp( chemName{1,k},'Acetaminophen')
        ...
        [p,n6,x] = fileparts( filepath.ace{6} );
    ...
    elseif strcmp( chemName{1,k},'H20')
        ...
        [p,n11,x] = fileparts( filepath.h20{11} );
        [p,n12,x] = fileparts( filepath.h20{12} );
        [p,n13,x] = fileparts( filepath.h20{13} );
      ...
      elseif strcmp( chemName{1,k},'GLY')
        ...
        [p,n1,x] = fileparts( filepath.gly{1} );
        [p,n2,x] = fileparts( filepath.gly{2} );
        [p,n3,x] = fileparts( filepath.gly{3} );
        [p,n4,x] = fileparts( filepath.gly{4} );
        [p,n5,x] = fileparts( filepath.gly{5} );
        [p,n6,x] = fileparts( filepath.gly{6} );
        [p,n7,x] = fileparts( filepath.gly{7} );
        [p,n8,x] = fileparts( filepath.gly{8} );
...
names={'bic','car','per','da','per50','ace', 'h20','tri','gly'};
count=0;
for i=1:length(names)
        xlswrite(xlsfilestr, names{i}, 1, ['B' num2str(100+count)]);
        count=count+1;
    for j=1:length(filepath.(names{i}) )

        xlswrite(xlsfilestr,filepath.(names{i})(j) , 1, ['B' num2str(100+count)]);
        count=count+1;
    end;
